			...
			module vendor
				module Delete
					def self.registered(app)
						...
						# DELETE /vendors/(0-9)
						app.delete %r{/vendors/([a-zA-Z0-9]*)} do
							# guest not allowed
							if !session[:user]
								halt 401
							end
							...
							# get all vendors
							document = Model::vendor.find(params[:captures].first)
					...
					def self.registered(app)
						...
						app.get uri do
							...
							if @request_payload[:web_enabled]
								vendors = Model::vendor.where(:logo.ne => nil, :web_enabled => true).sort(:name.asc)
							else
								vendors = Model::vendor.sort(:name.asc)
							end
						...
						# GET /vendors/search
						app.get %r{/vendors/search} do
							# get all vendors
							vendors = Model::vendor.where(:name => { :$regex => /#{@request_payload[:name]}/i }).sort(:name.asc)
						...
						# GET /vendors/(0-9)
						app.get %r{/vendors/([\-_a-zA-Z0-9]*)} do
							# get document
							document = Model::vendor.first(:id => params[:captures].first)
							if !document
								document = Model::vendor.first(:url => params[:captures].first)
							end
				...
				module Patch
					def self.registered(app)
						...
						# PATCH /vendors/(0-9)
						app.patch %r{/vendors/([a-zA-Z0-9]*)} do
							# guest not allowed
							if !session[:user]
								halt 401
							end
